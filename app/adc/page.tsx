  ...
  return (
    <DocCard>
      ...
      <DUl items={[
        ...
        'Examiners：機器讀真實來源；district_badges/group_badges 存 full_title 清單；unit 用「港島第XX旅」。',
        'ExaminerAppointments：申請暫存；notes 以「標籤：值 | …」存細節，含 badge_scopes(JSON)。',
        'Groups：旅團；申請存 group_id，寫入時由 normalizeUnit_ 轉「港島第XX旅」。',
      ]} />
      ...
      <DOl items={[
        ...
        '主考重複行：確保 Matrix/Examiners 姓名一致(含稱謂)、單位用「港島第XX旅」，再同步。',
      ]} />
...
function ScriptDoc() {
  return (
    <DocCard>
      ...
      <DSub>關鍵工具函式</DSub>
      <DUl items={[
        'normalizeUnit_：group_id → 「港島第XX旅」。',
      ...
      <DUl items={[
        ...
        'Examiners 章名存 full_title、unit 存「港島第XX旅」。',
        '主考姓名在三表須完全一致(含稱謂)。',
